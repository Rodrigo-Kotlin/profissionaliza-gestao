...
-- ============================================================================
-- Propósito:
--   Aplicar ao banco DEV o conteúdo CORRIGIDO da migration já registrada
--   20260912100000_phase2_4_crm_lead_identity_safety.sql (commit 8a9626a).
--   O banco remoto foi migrado com a versão anterior do arquivo (b355657) e
--   ainda possuía o helper antigo `_crm_name_overlap` e o create_crm_lead v1
--   (com semântica POSSIBLE_DUPLICATE para CPF/nome).
--
-- Escopo: APENAS objetos de função alterados por essa migration.
--   Nenhuma tabela/structure/RBAC/GRANT de schema novo. Nenhuma migration
--   nova — é um schema catch-up para homologação do fluxo E2E.
--
-- Uso (DEV, único projeto da org lkkdfzezwsbeuxllclct = epjshcgsjvrydwuyqixi):
...
-- ---------------------------------------------------------------------------
-- 1b. Remove helper antigo orfão (criado pela versão anterior da migration,
--     não existe mais no repositório e não é referenciado por ninguém)
-- ---------------------------------------------------------------------------
drop function if exists public._crm_name_overlap(text, text);
