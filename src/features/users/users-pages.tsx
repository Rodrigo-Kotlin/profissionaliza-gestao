...
import { can, canAny, PERMISSIONS } from '@/lib/rbac'
...
export function UsersPage() {
  ...
  const allowed = canAny(permissions, PERMISSIONS.USERS_VIEW, PERMISSIONS.USERS_MANAGE)
  ...
  if (!allowed) return <Card><EmptyState icon={ShieldCheck} title="Acesso restrito" description="Solicite acesso a um administrador do sistema." /></Card>
  return <div className="space-y-8"><PageHeader title="Usuários" description="Consulte os usuários e seus estados de acesso."><Button disabled={!can(permissions, PERMISSIONS.USERS_MANAGE)}>Gerenciar papéis</Button></PageHeader><Card className="overflow-hidden">{query.isLoading ? <div className="space-y-3 p-5">{Array.from({ length: 4 }, (_, i) => <Skeleton key={i} className="h-14" />)}</div> : query.isError ? <EmptyState icon={UserRoundX} title="Não foi possível carregar usuários" description="Tente novamente em alguns instantes." /> : !query.data?.length ? <EmptyState icon={Users} title="Nenhum usuário encontrado" description="Os usuários criados no Supabase Auth aparecerão aqui." /> : <DataTable data={query.data} getKey={(row) => row.id} columns={[{ key: 'user', header: 'Usuário', cell: (row) => <div className="flex items-center gap-3"><Avatar name={row.full_name} src={row.avatar_url} /><div className="min-w-0"><p className="truncate font-semibold">{row.full_name}</p><p className="truncate text-xs text-muted">{row.email}</p></div></div> }, { key: 'status', header: 'Status', cell: (row) => <Badge variant={row.is_active ? 'success' : 'danger'}>{row.is_active ? 'Ativo' : 'Inativo'}</Badge> }, { key: 'created', header: 'Criado em', priority: 'low', cell: (row) => new Date(row.created_at).toLocaleDateString('pt-BR') }]} />}</Card></div>
}
